...
df_heatmap<-read.csv("HUMAnN3_results/pathabundance_filtered_normalized.tsv", sep = "\t")

df_heatmap_long <- melt(df_heatmap, id.vars = "Pathway", variable.name = "Sample", value.name = "Abundance")%>%
...
heatmap <- ggplot(df_heatmap_long, aes(x = Sample, y = Pathway, fill = Abundance)) +
  ...
  scale_fill_gradientn(colors = c("white", "#D8BFD8", "purple", "#4B0082", "lightgreen", "green", "darkgreen"), 
...
ggsave("images/HUMAnN3_heatmap/heatmap_pathabundance.png", plot = heatmap, width = 12, height = 10, dpi=600)
