...
shannon_res <- wilcox.test(shannon ~ status, data = Alpha, correct=FALSE, exact=FALSE)
chao1_res <- wilcox.test(S.chao1 ~ status, data = Alpha, correct=FALSE, exact=FALSE)
eveness_res <- wilcox.test(eveness ~ status, data = Alpha, correct=FALSE, exact=FALSE)
...
shannon <- Alpha %>%
  mutate(status=factor(status, levels=c("Vespertilio murinus",
                                        "Nyctalus noctula"))) %>%
  ggplot(aes(x=status, y=shannon, fill = status))+
  geom_boxplot(width=0.25, size=0.5, shape=21, color="black", show.legend = FALSE)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_fill_manual(name = "Bat species",
  ...
  annotate(geom="richtext", x=1.5, y=6, 
           label= "M-W, <i>p</i>-value=0.075", size=4.5, label.color = NA)+
...
chao1 <- Alpha %>%
  ...
  theme(axis.text.x =element_blank(),
        ...
        legend.key=element_blank()) + 
  theme(text = element_text(size = 14)) +
  annotate(geom="richtext", x=1.5, y=12600, 
           label= "M-W, <i>p</i>-value=0.028", size=4.5, label.color = NA)+
  theme(axis.title.x=element_blank(),
        legend.text = element_markdown(face = "italic"))+
  labs(y= "Chao1 index")
...
pielou <- Alpha %>%
  ...
  theme(axis.text.x =element_blank(),
        ...
        legend.key=element_blank()) + 
  theme(text = element_text(size = 14)) +
  annotate(geom="richtext", x=1.5, y=0.65, 
           label= "M-W, <i>p</i>-value=0.075", size=4.5, label.color = NA)+
  theme(axis.title.x=element_blank(),
        legend.text = element_markdown(face = "italic"))+
  labs(y= "Pielou index")
...
metadata <- read.table('metadata.csv', 
  ...
  mutate(status = factor(Group),
         status = fct_relevel(Group, "Vespertilio murinus",
                              "Nyctalus noctula"))

taxon_counts <- read.table('counts/csv/counts_species.csv', sep=',', comment='', row.names=1)
...
PCOA_bray_plot <- positions_bray %>%
  ...
  scale_color_manual(name="Bat species", 
                     breaks=c("Vespertilio murinus",
                              "Nyctalus noctula"),
                     labels=c("Vespertilio murinus",
                              "Nyctalus noctula"),
                     values=c(alpha("#0000FF", 0.5), alpha("#FF0000", 0.5), alpha("gray", 0.5)))+
  stat_ellipse(show.legend=FALSE)+
  annotate(geom='richtext', x=0.5, y=0.8, 
...
PCOA_jaccard_plot <- positions_jaccard %>%
  ...
  scale_color_manual(name="Bat species", 
                     ...
                     values=c(alpha("#0000FF", 0.5), alpha("#FF0000", 0.5), alpha("gray", 0.5)))+
  stat_ellipse(show.legend=FALSE)+
  annotate(geom='richtext', x=0.5, y=0.5, 
           label= "PERMANOVA<br><i>p</i>-value=0.047", 
           size=4, fill = NA, label.color = NA)+
  theme_classic()+
  theme(legend.text = element_text(face = "italic"),
...
ggsave("images/alpha_beta_div/combined.png", plot = combined, width = 14, height = 8, dpi=600)
