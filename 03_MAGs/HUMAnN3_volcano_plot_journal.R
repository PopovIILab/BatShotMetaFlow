...
DAA_pathways$diffabund[DAA_pathways$coef < -1 & DAA_pathways$qval < 0.05 & DAA_pathways$metadata == "Group"] <- "NN_minus"
DAA_pathways$feature[DAA_pathways$diffabund == "NO"] <- NA
DAA_pathways$label <- NA
top_red <- DAA_pathways[DAA_pathways$coef < -1 & DAA_pathways$qval < 0.05 & DAA_pathways$metadata == "Group", ]
...
DAA_pathways$label[DAA_pathways$coef > 1 & DAA_pathways$qval < 0.05 & DAA_pathways$metadata == "Group"] <- DAA_pathways$feature[DAA_pathways$coef > 1 & DAA_pathways$qval < 0.05 & DAA_pathways$metadata == "Group"]

volcano_plot <- ggplot(data = DAA_pathways, aes(x = coef, y = -log10(qval), label = feature, col = diffabund)) +
  geom_point(size = 2) +
  theme_bw() +
  geom_vline(xintercept = c(-1, 1), col = "gray", linetype = 'dashed') +
  geom_hline(yintercept = c(-log10(0.05)), col = "gray", linetype = 'dashed') +
  geom_text_repel(data = subset(DAA_pathways, !is.na(label)),
                  aes(label = label),
                  size = 2,
                  box.padding = 1,
                  point.padding = 0.5,
...
ggsave("images/volcano_plots/volcano_plot.png", plot = volcano_plot, width = 10, height = 6, dpi = 600)
