...
df_card <- read.csv("ABRicate_results/summary/presence/card_presence.tsv", sep = "\t", check.names = FALSE)%>%
  filter(SAMPLE != "D4")%>%
...
df_card_long <- melt(df_card, id.vars = "SAMPLE", variable.name = "ARG", value.name = "presence")%>%
...
ggsave("images/ABRicate_heatmaps/heatmap_card.png", plot = heatmap_card, width = 6, height = 16, dpi=600)

#############
# RESFINDER #
#############

df_resfinder <- read.csv("ABRicate_results/summary/presence/resfinder_presence.tsv", sep = "\t", check.names = FALSE)%>%
  filter(SAMPLE != "D4")%>%
  mutate(SAMPLE = str_replace(SAMPLE, "D1", "VM1"),
         SAMPLE = str_replace(SAMPLE, "D2", "VM2"),
         SAMPLE = str_replace(SAMPLE, "D3", "VM3"),
...
df_resfinder_long <- melt(df_resfinder, id.vars = "SAMPLE", variable.name = "ARG", value.name = "presence")%>%
  filter(ARG != "NUM FOUND")
...
ggsave("images/ABRicate_heatmaps/heatmap_resfinder.png", plot = heatmap_resfinder, width = 6, height = 8, dpi=600)

########
# VFDB #
########

df_vfdb <- read.csv("ABRicate_results/summary/presence/vfdb_presence.tsv", sep = "\t", check.names = FALSE)%>%
  filter(SAMPLE != "D4")%>%
  filter(SAMPLE != "P3")%>%
  mutate(SAMPLE = str_replace(SAMPLE, "D1", "VM1"),
         SAMPLE = str_replace(SAMPLE, "D2", "VM2"),
...
df_vfdb_part1_long <- melt(df_vfdb_part1, id.vars = "SAMPLE", variable.name = "ARG", value.name = "presence")
...
df_vfdb_part2_long <- melt(df_vfdb_part2, id.vars = "SAMPLE", variable.name = "ARG", value.name = "presence")
...
df_vfdb_part3_long <- melt(df_vfdb_part3, id.vars = "SAMPLE", variable.name = "ARG", value.name = "presence")
...
combined <- heatmap_vfdb_part1 + heatmap_vfdb_part2 + heatmap_vfdb_part3 & plot_annotation(tag_levels = list(c("A", "B", "C")))
ggsave("images/ABRicate_heatmaps/heatmap_vfdb_all.png", plot = combined, width = 20, height = 16, dpi=600)

ggsave("images/ABRicate_heatmaps/heatmap_vfdb1.png", plot = heatmap_vfdb_part1, width = 5, height = 16, dpi=600)
