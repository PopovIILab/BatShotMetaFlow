...
metadata <- read.table('phylogenomics/metadata/well_done_metadata.tsv', sep='\t', header=T)
...
NSP <- ggtree(NSP_tree) %<+% metadata + 
  geom_tiplab(image=df_NSP$uid,
              geom="phylopic",
              size = ifelse(df_NSP$name == "NC_040626.1", 
                            0.015, 
                            ifelse(df_NSP$name == "NC_005041.2" |
                                     df_NSP$name == "NC_005040.2" |
                                     df_NSP$name == "NC_026943.1" |
                                     df_NSP$name == "NC_001899.1" |
                                     df_NSP$name == "NC_077036.1", 0.02,
                                   ifelse(df_NSP$name == "NC_077050.1", 0.01,
                                          ifelse(df_NSP$uid == "6e42ac62-37f4-458c-bf44-3f7decb1f14e", 0.007, 0.03)))),
              offset = 1.5,
              align=TRUE) +
  xlim(0,4.7) + 
  geom_tiplab(
...
NSP <- NSP + geom_tiplab(aes(image = flag_path), geom = "image", offset = 1.8, align = TRUE, size = 0.02, linesize = 0)

NSP <- gheatmap(NSP, meta.year, width=0.05, offset=2, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_viridis(option="D", name="Year", discrete = TRUE, na.translate = T)

NSP <- NSP + 
  annotate("text", x=max(NSP$data$x) + 1.5, y=-0.05, label="Host", size=4) +
  annotate("text", x=max(NSP$data$x) + 1.8, y=-0.05, label="Country", size=4)

ggsave('images/NSP_tree.png', NSP, width=14, height=12, dpi=600)
...
SP <- ggtree(SP_tree) %<+% metadata + 
  geom_tiplab(image=df_SP$uid,
               geom="phylopic",
               size = ifelse(df_SP$name == "NC_004290.1", 0.012, 0.05),
               offset = 1.5,
               align=TRUE) +
  xlim(0,4.2) + 
  geom_tiplab(
...
SP <- SP + geom_tiplab(aes(image = flag_path), geom = "image", offset = 1.8, align = TRUE, size = 0.04, linesize = 0)

SP <- gheatmap(SP, meta.year, width=0.05, offset=2, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_viridis(option="D", name="Year", discrete = TRUE, na.translate = T)

SP <- SP + 
  annotate("text", x=max(SP$data$x) + 1.5, y=-0.05, label="Host", size=4) +
  annotate("text", x=max(SP$data$x) + 1.8, y=-0.05, label="Country", size=4)

ggsave('images/SP_tree.png', SP, width=14, height=6, dpi=600)
...
All <- ggtree(All_tree) %<+% metadata + 
  geom_tiplab(image=df_All$uid,
              geom="phylopic",
              size = ifelse(df_All$name == "NC_004290.1", 0.012, 0.05),
              offset = 1.5,
              align=TRUE) +
  xlim(0,4.2) + 
  geom_tiplab(
...
All <- All + geom_tiplab(aes(image = flag_path), geom = "image", offset = 1.8, align = TRUE, size = 0.04, linesize = 0)

All <- gheatmap(All, meta.year, width=0.05, offset=2, color="black", font.size=4, colnames_offset_y = -0.05) + 
  scale_fill_viridis(option="D", name="Year", discrete = TRUE, na.translate = T)

All <- All + 
  annotate("text", x=max(All$data$x) + 1.5, y=-0.05, label="Host", size=4) +
  annotate("text", x=max(All$data$x) + 1.8, y=-0.05, label="Country", size=4)

ggsave('images/All_tree.png', All, width=14, height=6, dpi=600)
...
combined <- ((All / SP + plot_layout(guides = 'collect')) | NSP) & plot_annotation(tag_levels = list(c("A", "B", "C")))

ggsave("images/combo_tree.png", plot = combined, width = 28, height = 12, dpi=600)
